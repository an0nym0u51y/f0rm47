...
impl Decode for IpAddr {
    fn decode_with_len_from<R: Read>(mut reader: R) -> Result<(Self, usize), Self::Error> {
        let (tag, read1) = u8::decode_with_len_from(&mut reader)?;
        match tag {
            4 => {
                let (addr, read2) = Ipv4Addr::decode_with_len_from(&mut reader)?;
                Ok((IpAddr::V4(addr), read1 + read2))
            },
            6 => {
                let (addr, read2) = Ipv6Addr::decode_with_len_from(&mut reader)?;
...
impl Decode for SocketAddr {
    fn decode_with_len_from<R: Read>(mut reader: R) -> Result<(Self, usize), Self::Error> {
        ...
        match tag {
            4 => {
                let (addr, read2) = SocketAddrV4::decode_with_len_from(&mut reader)?;
                Ok((SocketAddr::V4(addr), read1 + read2))
            },
            6 => {
                let (addr, read2) = SocketAddrV6::decode_with_len_from(&mut reader)?;
...
impl Decode for Ipv4Addr {
    fn decode_with_len_from<R: Read>(reader: R) -> Result<(Self, usize), Self::Error> {
        let (bytes, len) = <[u8; 4]>::decode_with_len_from(reader)?;
...
impl Decode for Ipv6Addr {
    fn decode_with_len_from<R: Read>(reader: R) -> Result<(Self, usize), Self::Error> {
        let (bytes, len) = <[u8; 16]>::decode_with_len_from(reader)?;
        Ok((Self::from(bytes), len))
    }
}
...
impl Decode for SocketAddrV4 {
    fn decode_with_len_from<R: Read>(mut reader: R) -> Result<(Self, usize), Self::Error> {
        let (ip, read1) = Ipv4Addr::decode_with_len_from(&mut reader)?;
        let (port, read2) = u16::decode_with_len_from(&mut reader)?;
...
impl Decode for SocketAddrV6 {
    fn decode_with_len_from<R: Read>(mut reader: R) -> Result<(Self, usize), Self::Error> {
        let (ip, read1) = Ipv6Addr::decode_with_len_from(&mut reader)?;
        let (port, read2) = u16::decode_with_len_from(&mut reader)?;
        let (flowinfo, read3) = u32::decode_with_len_from(&mut reader)?;
        let (scope_id, read4) = u32::decode_with_len_from(&mut reader)?;
